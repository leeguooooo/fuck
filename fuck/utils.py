...
def get_all_executables():
    ...
    fuck_alias = get_alias()
    fuck_entry_points = ['fuck']
    ...
    bins = [exe.name.decode('utf8') if six.PY2 else exe.name
            ...
            and exe.name not in fuck_entry_points]
    aliases = [alias.decode('utf8') if six.PY2 else alias
               for alias in shell.get_aliases() if alias != fuck_alias]
...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('fuck').as_posix()
...
def get_installation_version():
    def _from_importlib():
        ...
        try:
            dist_names = packages_distributions().get('fuck', [])
        except Exception:
            dist_names = []
...
def get_valid_history_without_current(command):
    def _not_corrected(history, alias):
        ...
        previous = None
        for line in history:
            if previous is not None and line != alias:
                yield previous
            previous = line
        if history:
            yield history[-1]
    ...
    return [line for line in _not_corrected(history, fuck_alias)
            if not line.startswith(fuck_alias) and not line == command.script
