...
def _get_not_configured_usage_tracker_path():
    ...
    return Path(gettempdir()).joinpath(u'fuck.last_not_configured_run_{}'.format(
        getpass.getuser(),
    ))
...
def main():
    ...
    # Check if we're being called through the alias (FUCK_COMMAND is set)
    if os.environ.get('FUCK_COMMAND'):
