...
class TestTcsh(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('fuck.shells.tcsh.Popen')
        mock.return_value.stdout.read.return_value = (
            b'fuck\teval `\\fuck ${fucked_cmd}`\n'
            b'l\tls -CF\n'
            b'la\tls -A\n'
            b'll\tls -alF')
        return mock
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'fuck': 'eval `\\fuck ${fucked_cmd}`',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}

    def test_app_alias(self, shell):
        assert 'setenv FUCK_SHELL tcsh' in shell.app_alias('fuck')
