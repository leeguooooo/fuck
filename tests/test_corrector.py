...
from fuck import corrector, const
from fuck.system import Path
from fuck.types import Command
from fuck.corrector import get_corrected_commands, organize_commands


@pytest.fixture
def glob(mocker):
    results = {}
    mocker.patch('fuck.system.Path.glob',
...
class TestGetRules(object):
    ...
    def load_source(self, monkeypatch):
        monkeypatch.setattr('fuck.types.load_source',
                            lambda x, _: Rule(x))
...
def test_get_rules_rule_exception(mocker, glob):
    load_source = mocker.patch('fuck.types.load_source',
...
def test_get_corrected_commands(mocker):
    ...
    rules = [Rule(match=lambda _: False),
             ...
             Rule(match=lambda _: True,
                  ...
                  priority=60)]
    mocker.patch('fuck.corrector.get_rules', return_value=rules)
