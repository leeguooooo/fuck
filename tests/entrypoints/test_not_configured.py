...
from fuck.entrypoints.not_configured import main


@pytest.fixture(autouse=True)
def usage_tracker(mocker):
    return mocker.patch(
        'fuck.entrypoints.not_configured._get_not_configured_usage_tracker_path',
...
def shell_pid(mocker):
    return mocker.patch('fuck.entrypoints.not_configured._get_shell_pid',
                        new_callable=MagicMock)


@pytest.fixture(autouse=True)
def shell(mocker):
    shell = mocker.patch('fuck.entrypoints.not_configured.shell',
                         new_callable=MagicMock)
...
def shell_config(mocker):
    path_mock = mocker.patch('fuck.entrypoints.not_configured.Path',
                             new_callable=MagicMock)
...
@pytest.fixture(autouse=True)
def logs(mocker):
    return mocker.patch('fuck.entrypoints.not_configured.logs',
                        new_callable=MagicMock)
