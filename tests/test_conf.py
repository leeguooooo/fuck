...
from fuck import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('fuck.conf.load_source')
...
class TestSettingsFromEnv(object):
    def test_from_env(self, os_environ, settings):
        os_environ.update({'FUCK_RULES': 'bash:lisp',
                           'FUCK_EXCLUDE_RULES': 'git:vim',
                           'FUCK_WAIT_COMMAND': '55',
                           'FUCK_REQUIRE_CONFIRMATION': 'true',
                           'FUCK_NO_COLORS': 'false',
                           'FUCK_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                           'FUCK_WAIT_SLOW_COMMAND': '999',
                           'FUCK_SLOW_COMMANDS': 'lein:react-native:./gradlew',
                           'FUCK_NUM_CLOSE_MATCHES': '359',
                           'FUCK_EXCLUDED_SEARCH_PATH_PREFIXES': '/media/:/mnt/'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, os_environ, settings):
        os_environ.update({'FUCK_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/fuck'),
    (False, '/user/test/config/', '/user/test/config/fuck'),
    (True, '~/.config', '~/.fuck'),
    (True, '/user/test/config/', '~/.fuck')])
def test_get_user_dir_path(mocker, os_environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('fuck.conf.Path.is_dir',
