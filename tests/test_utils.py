...
from fuck.utils import default_settings, \
...
class TestGetCloseMatches(object):
    @patch('fuck.utils.difflib_get_close_matches')
...
def get_aliases(mocker):
    mocker.patch('fuck.shells.shell.get_aliases',
...
def test_get_all_executables_pathsep(path, pathsep):
    with patch('fuck.utils.Path') as Path_mock:
        get_all_executables()
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('fuck.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        monkeypatch.setattr('fuck.utils.cache.disabled', False)
        _cache._init_db()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('fuck.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('fuck.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        mock = mocker.patch('fuck.shells.shell.get_history')
    ...
    def alias(self, mocker):
        return mocker.patch('fuck.utils.get_alias',
                            return_value='fuck')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('fuck.utils.Path', return_value=path_mock)
